...
const FILTERS: FilterType[] = ['Todas', 'Programadas', 'Realizadas', 'Canceladas'];
...
const MisCitas = () => {
  ...
  const filteredProximas = CITAS.filter(c =>
    c.status === 'Programada' && (filter === 'Todas' || filter === 'Programadas')
  );
    ...
    <div className="mc-layout">
      ...
      <aside className="mc-sidebar">
        <div className="mc-logo">
          <span>iParty</span><span className="mc-logo-gold">DJs</span>
          <div className="mc-logo-sub">Portal del cliente</div>
        </div>

        <div className="mc-nav-group">
          <div className="mc-nav-label">Mi cuenta</div>
          {['Dashboard', 'Mis solicitudes', 'Mis citas', 'Mis eventos', 'Mis reseñas'].map(item => (
            <a key={item} href="#" className={`mc-nav-item ${item === 'Mis citas' ? 'active' : ''}`}>
              {item}
            </a>
          ))}
        </div>
        ...
        <div className="mc-nav-group">
          ...
          {['Galería', 'Contacto'].map(item => (
            <a key={item} href="#" className="mc-nav-item">{item}</a>
          ))}
        </div>
      ...
      <main className="mc-main">
        ...
        <div className="mc-header">
          ...
          <p className="mc-subtitle">Reuniones virtuales programadas con el equipo de iParty DJs</p>
        </div>
          ...
          <>
            ...
            <div className="mc-list">
              {filteredProximas.map(cita => (
                <div key={cita.id} className="mc-item mc-item-proxima">
                  <div className="mc-date-col">
                    <span className="mc-dia">{cita.dia}</span>
                    <span className="mc-mes">{cita.mes}</span>
                  </div>
                  <div className="mc-accent-bar" />
                  <div className="mc-content">
                    <div className="mc-cita-title">{cita.titulo}</div>
                    <div className="mc-cita-meta">
                      <span>{cita.horaInicio} — {cita.horaFin}</span>
                      <span className="mc-anfitrion">{cita.anfitrion}</span>
                    </div>
                    {cita.link && (
                      <a href={`https://${cita.link}`} className="mc-btn-join" target="_blank" rel="noopener noreferrer">
                        Unirse a la videollamada
                      </a>
                    )}
                  </div>
                  <span className={`mc-badge ${STATUS_MAP[cita.status]}`}>{cita.status}</span>
                </div>
              ))}
            </div>
          </>
        ...
        {filteredHistorial.length > 0 && (
          <>
            <div className="mc-section-label mc-section-label-mt">Historial</div>
            <div className="mc-list">
              {filteredHistorial.map(cita => (
                <div key={cita.id} className="mc-item">
                  ...
                  <div className="mc-content">
                    ...
                    {cita.nota && <div className="mc-nota">{cita.nota}</div>}
                  </div>
                  <span className={`mc-badge ${STATUS_MAP[cita.status]}`}>{cita.status}</span>
                </div>
              ))}
        ...
        {filteredProximas.length === 0 && filteredHistorial.length === 0 && (
          <div className="mc-empty">No hay citas en esta categoría.</div>
        )}
      </main>
    </div>
  );
